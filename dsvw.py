...
def init():
    ...
    This function sets up reuse of connection addresses for the HTTP Server and creates a new database in memory with "users" and "comments" tables. The "users" table is filled with data parsed from the USERS_XML global constant.
    
    This function does not take any arguments and does not return anything.
    
    Globals:
        connection: sqlite3.Connection object used globaly for database operations.
        USERS_XML: An XML string that contains user data.
    
    Raises:
        sqlite3.Error: An error from sqlite3 if something goes wrong with SQL operations.
        xml.etree.ElementTree.ParseError: An error from xml.etree.ElementTree in case of invalid XML in USERS_XML.
    """
    global connection
...
class ReqHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        This method processes a GET request and sends a response. The response varies depending on the parameters given in the query. The parameters and their uses are:
        
        id: An SQL query involving this parameter's value is executed. The result is then added to the HTML content.
        v: A version number. The HTML postfix is adjusted to include this value.
        object: The value of this parameter is unpickled and then converted to a string.
        path: The file at the specified path is opened and its content is read and set as the HTML content.
        domain: An nslookup command is executed with this parameter's value and the result is set as the HTML content.
        xml: This parameter's value is parsed as XML and then converted back to a string.
        name: An XPath query is performed on the USERS_XML constant using this parameter's value.
        size: The server 'resizes an image' (simulated) to the dimensions specified by this parameter's value. The time it took is added to the HTML content.
        comment: An SQL query involving this parameter's value is executed. The result is then added to the HTML content.
        include: The file at this parameter's value's is executed as a Python script and its output is added to the HTML content.
        redir: The HTML content is manipulated to refresh and redirect to this parameter's value's path.
        callback: In the /users.json path, the JSON object is wrapped in a function call to this parameter's value's function.
        
        At the end of the operation, it sends back a HTTP response with an HTML or plaintext body, depending on whether the content starts with a DOCTYPE declaration.
        
        Args:
            self (object): a reference to the instance calling the method.
        
        This method does not explicitly return. It sends response and then ends, but technically returns None implicitly.
        """
...
class ThreadingServer(socketserver.ThreadingMixIn, http.server.HTTPServer):
    def server_bind(self):
        """Configures and binds the server socket.
        
        This method sets the socket options to allow address reuse and then calls the parent class's bind operation.
