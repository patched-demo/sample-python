            ...
            if path == '/':
                if "id" in params:
                    ...
                    if not "://" in params["path"]:
                        response = open(os.path.abspath(params["path"]), "rb").read().decode()
                    else:
                        response = urllib.request.urlopen(params["path"]).read().decode()
                    ...
                    content = tostring(parse(io.BytesIO(params["xml"].encode())), encoding='unicode', method='xml', xml_declaration=True, pretty_print=True)
                elif "name" in params:
                    from defusedxml.ElementTree import fromstring
                    import io
                    
                    root = fromstring(io.BytesIO(USERS_XML.encode()))
                    
                    found = root.findall(".//user[name/text()='%s']" % params["name"])
                    ...
                    backup, sys.stdout, program, envs = sys.stdout, io.StringIO(), (open(params["include"], "rb") if not "://" in params["include"] else requests.get(params["include"]).content), {"DOCUMENT_ROOT": os.getcwd(), "HTTP_USER_AGENT": self.headers.get("User-Agent"), "REMOTE_ADDR": self.client_address[0], "REMOTE_PORT": self.client_address[1], "PATH": path, "QUERY_STRING": query}
                    # Remove the vulnerable exec call
                ...
                cursor.execute("SELECT * FROM users WHERE username=? AND password=?", (re.sub(r"[^\w]", "", params.get("username", "")), params.get("password", "")))
