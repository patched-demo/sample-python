...
def init():
    """Initializes and sets up the in-memory database.
    
    This method creates a global sqlite3 database connection in memory. It sets the HTTPServer's allow_reuse_address parameter to True to permit the socket to be reused immediately after it is closed. It then generates a 'users' and 'comments' table and populates the 'users' table with data parsed from an XML string.
    
    The 'users' table consists columns: id, username, name, surname, password. The 'comments' table consists columns: id, comment, time.
    
    Note: This method doesn't accept any arguments and doesn't have a return value. The resulting connection object and cursor are global variables, and intended to be accessed directly from other methods.
...
class ReqHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        """Processes HTTP GET requests, by analyzing the path and query parameters.
        
        The method generates varying responses for different query parameters in the path such as 'id', 'v', 'object', 'path', 'domain', 'xml', 'name', 'size', 'comment', 'include', 'redir', and many more. It also handles exceptions and errors.
        
        Args:
            self (self): Implicit object parameter representing the instance being called. 
        
        No explicit return, the function writes to the wfile object property of the instance to generate a server response.
        """
...
class ThreadingServer(socketserver.ThreadingMixIn, http.server.HTTPServer):
    def server_bind(self):
        """Overrides the server_bind method to call the parent class's server_bind and set the socket options.
        
        This method allows the server to re-bind to a port that is still in TIME_WAIT state due to previous server instance's lingering connections.
